    ...
    for i, tc in enumerate(tool_calls):
        ...
        for prereq in prereqs:
            if prereq not in called_tools:
                ...
                all_called = {tc.get("name", "") for tc in tool_calls}
        ...
        #     不同工具有不同的必填参数：
        required_params = {
            "parse_prd": ["file_path"],  # 或 content
            "analyze_requirements": ["prd_json"],
            "extract_features": ["prd_json"],
            "extract_rules": ["prd_json"],
            "generate_cases": ["features_json", "rules_json"],
            "format_output": ["cases_json"],
        }
        req = required_params.get(tool_name, [])
        for param in req:
            if param not in params or not params[param]:
                errors.append(f"步骤{i}: {tool_name} 缺少参数 '{param}'")
                call_ok = False
...
def eval_step_efficiency(trace: dict) -> dict:
    ...
    #         prev = tool_calls[i-1]
    #         curr = tool_calls[i]
    #         if (prev.get("name") == curr.get("name") and
    #                 prev.get("parameters") == curr.get("parameters")):
    #             redundant.append({
    #                 "step": i,
    #                 "tool": curr.get("name"),
...
def eval_error_recovery(trace: dict) -> dict:
    ...
    #                 "error": result["error"],
    #             })
    #     # 也可能 error 是字符串（JSON 解析失败后的原始文本）
    ...
    #                 "step_index": i,
    #                 "tool_name": step.get("tool_name", "unknown"),
    #                 "error": result_str[:200],  # 截取前 200 字符
    #             })
    #
    # ============================================================
    ...
    #     # 检查是否重试了同一工具（可能修正了参数）
    #     retried = any(
    #         s.get("type") == "tool_call" and s.get("name") == err["tool_name"]
...
#      - tool_result 事件：检查是否返回 error，后续是否尝试修复
#   2. 对每一步分配状态：
#      - 大部分工具调用都是 Act（正常）
...
def classify_control_decision(trace: dict) -> dict:
    ...
    #         # 如果上一步是 error，当前调用可能是 Recover
    #         if prev_was_error:
    #             state = "Recover"
    #             reason = f"上一步出错后尝试恢复（调用 {step.get('name', '')}）"
    #         else:
    #             state = "Act"
    #             reason = f"调用工具 {step.get('name', '')}"
    #
    ...
    #         if is_error:
    #             prev_was_error = True
    #             state = "Act"  # result 本身不算决策，标记为 Act
    #             reason = "工具返回错误"
    #         else:
